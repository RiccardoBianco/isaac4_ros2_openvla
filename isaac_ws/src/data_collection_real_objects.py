...
objects = {
    ...
    "cracker_box": {
        ...
        "specific_offset_ee": 0.05,  # Specific offset for the object
...
SAMPLES_PER_OBJECT = 300 # Number of samples per object
...
INIT_TARGET_POS = [0.50, -0.3, 0.0]  # Z must be 0 in OpenVLA inference script
INIT_ROBOT_POSE = [0.4, 0.0, 0.35, 0.0, 1.0, 0.0, 0.0]

OPENVLA_INSTRUCTION = f"Pick the {OBJECTS[OBJECT_TO_PICK]} and place it on the red area. \n" 
...
OBJECT_SIZE = [0.08, 0.08, 0.08]  # Size of the cube
...
OFFSET_EE = OBJECT_SIZE[2] / 2 + SPECIFIC_OFFSET_EE + 0.107 # 0.01 settato a mano a naso
    ...
    def __post_init__(self):
        ...
        self.scene.box = RigidObjectCfg(
            ...
            spawn=sim_utils.CuboidCfg(
                size=(0.4, 0.10, 0.0025),  # Dimensioni del cubo
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            if dones.any():
                ...
                sm.reset()
                task_count += 1
                if task_count >= 2*SAMPLES_PER_OBJECT:
                    object_to_pick = "object3"
                elif task_count >= SAMPLES_PER_OBJECT:
                    object_to_pick = "object2"
                
                if task_count > 3 * SAMPLES_PER_OBJECT:
