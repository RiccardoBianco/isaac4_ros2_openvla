...
class OpenVLAServer:
    def __init__(self, openvla_path: Union[str, Path], attn_implementation: Optional[str] = "flash_attention_2") -> Path:
        ...
        # Load VLA Model using HF AutoClasses and then change parameters using the finetuned .safetensors
        ...
        # TODO understand where to put the .safetensors file
        # 🔁 Load fine-tuned adapter weights (partial or full)
        adapter_weights = load_file("adapter_model.safetensors")

        # Update base model with adapter weights
        missing, unexpected = self.vla.load_state_dict(adapter_weights, strict=False)

        # (Optional) Log what was loaded or skipped
        print(f"Loaded adapter weights. Missing keys: {len(missing)}, Unexpected keys: {len(unexpected)}")

        # TODO add support for dataset_statistics.json
